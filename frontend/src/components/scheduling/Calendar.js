...
const updateCustomEvents = (
  ...
  firstCustomEventDataLoaded
) => {
  if (!calendarApi) {
    return;
  }
  const eventsToAdd = customEvents.filter(
    ({ id }) =>
      !customEventsIds.find((calendarEventId) => calendarEventId === id)
  );
  const eventsToRemove = customEventsIds.filter(
    (calendarEventId) =>
      !customEvents.find((customEvent) => customEvent.id === calendarEventId)
  );
  for (const eventToAdd of eventsToAdd) {
    const eventObject = {
      id: eventToAdd.id,
      color: eventToAdd.customColor
        ? eventToAdd.customColor
        : isPreview && !user?.showCalendarEventColors
        ? null
        : eventToAdd.gcalId
        ? "orange"
        : eventToAdd.isScheduledMeeting
        ? "red"
        : eventToAdd.isFreeBlock
        ? "green"
        : "purple",
    ...
    if (eventToAdd.isRecurring) {
      ...
      if (eventToAdd.gcalId) {
        const dtstart = new Date(eventToAdd.startDate)
          .toISOString()
          .replaceAll(/\.(.+?)Z/g, "Z")
          .replaceAll(/-|:/g, "");
        eventObject.rrule = `DTSTART:${dtstart}\n${eventToAdd.rrule}`;
        eventObject.duration =
          new Date(eventToAdd.endDate).getTime() -
          new Date(eventToAdd.startDate).getTime();
      } else {
...
const updateIcsEventSources = (
  username,
  icsFiles,
  icsEventSourcesIds,
  setIcsEventSourcesIds,
  calendarApi,
  ...
  const icsToAdd = icsFiles.filter(
    ({ s3IcsId }) =>
      !icsEventSourcesIds.find((eventSourceId) => eventSourceId === s3IcsId)
  );
  const icsToRemove = icsEventSourcesIds.filter(
    (s3IcsId) => !icsFiles.find((icsFile) => icsFile.s3IcsId === s3IcsId)
  );
  ...
  for (const { s3IcsId } of icsToAdd) {
    calendarApi.addEventSource({
      ...
      url: getServerUrl(
        `calendar${
          username ? `/${username}` : ""
        }/ics/${s3IcsId}?accessToken=${encodeURIComponent(
...
function Calendar({
  ...
  const [showGCalsManagementModal, setShowGCalsManagementModal] =
    useState(false);
  ...
  const [firstCustomEventDataLoaded, setFirstCustomEventDataLoaded] =
    useState(false);
    ...
    if (previewIcsFiles) {
      ...
      get(`calendar${isPreview ? `/${user.username}` : ""}/ics`, (result) => {
        if (result?.data?.length) {
          ...
          setIcsFiles([
            ...result.data.map(({ ics_name, s3_ics_id }) => ({
              icsName: ics_name,
              s3IcsId: s3_ics_id,
            })),
          ]);
      ...
      get(
        `calendar${isPreview ? `/${user.username}` : ""}/custom-events`,
        (result) => {
          if (result?.data?.length) {
            setFirstCustomEventDataLoaded(true);
            setCustomEvents([...result.data.map(parseCustomEventPayload)]);
  ...
  useEffect(() => {
    updateIcsEventSources(
      isPreview ? user?.username : null,
      icsFiles,
      icsEventSourcesIds,
      setIcsEventSourcesIds,
    ...
    <div className="schedule">
      <FullCalendar
        ...
        eventClick={(info) => {
          ...
          if (mergedCustomEvents) {
            ...
            if (!showAddEventModal && info.event.extendedProps.isCustomEvent) {
              setEditedEvent(
                customEvents.find((ev) => ev.id === info.event.id)
              );
        ...
        eventChange={(info) => {
          ...
          const daysDragged =
            info.event.start.getDay() - info.oldEvent.start.getDay();
          if (
            !ev ||
            eventEditPending ||
            (ev.isRecurring &&
              info.event.end.getDay() - info.oldEvent.end.getDay() !==
                daysDragged)
          ) {
            info.revert();
            return;
          ...
          post(
            "calendar/custom-event",
            {
              ...
              startDate:
                !ev.isAllDay && ev.startDate
                  ? new Date(new Date(ev.startDate).getTime() + startTimeDiff)
                  : null,
              endDate:
                !ev.isAllDay && ev.endDate
                  ? new Date(new Date(ev.endDate).getTime() + endTimeDiff)
                  : null,
              startRecurDate:
                ev.isRecurring && ev.recurStartDate
                  ? new Date(
                      new Date(ev.recurStartDate).getTime() + startTimeDiff
                    )
                  : null,
              endRecurDate:
                ev.isRecurring && !ev.isEndless && ev.recurEndDate
                  ? new Date(new Date(ev.recurEndDate).getTime() + endTimeDiff)
                  : null,
              recurDays: ev.recurDays?.map((day) =>
                (day + daysDragged) % 7 < 0
                  ? 7 + ((day + daysDragged) % 7)
                  : (day + daysDragged) % 7
              ),
            ...
            (result) => {
              ...
              setCustomEvents([
                ...customEvents
                  .slice(0, idx)
                  .concat(customEvents.slice(idx + 1)),
                parseCustomEventPayload(result.data),
              ]);
        ...
        customButtons={{
          ...
          schedule:
            isPreview && !previewIcsFiles && !mergedCustomEvents
              ? {
        ...
        headerToolbar={{
          left: isPreview
            ? previewIcsFiles || mergedCustomEvents
              ? ""
              : "schedule"
            : "addEvent manageIcs manageGCals",
          right: mergedCustomEvents
            ? "customPrev,customNext"
            : `prev,next today${
                isPreview && (user || previewIcsFiles) ? "" : " privacy"
              }`,
          center: "title",
        }}
      />
