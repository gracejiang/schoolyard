...
function Schedule({ forceRecreateKey, setForceRecreateKey }) {
  ...
  const [loadingUserCalendarEvents, setLoadingUserCalendarEvents] =
    useState(false);
  const [currentlyCalendarProcessingUser, setCurrentlyCalendarProcessingUser] =
    useState();
  ...
  useEffect(() => {
    ...
    get(`user/profile`, (result) => {
      if (result?.data?.username) {
        ...
        usernames = Array.from(new Set(usernames)).filter(
        ...
        const loadUser = () => {
          get(
            `user/profile${usernames[i] ? `/${usernames[i]}` : ""}`,
            (result) => {
  ...
  useEffect(() => {
    if (
      ...
      setTimeout(() => {
        ...
        const calendarWeekEvents = exposedCalendarApi
          .getEvents()
          .filter(
            (ev) =>
              ev?.end?.getTime() >= weekStart.getTime() &&
              ev?.start?.getTime() <= weekEnd.getTime()
          )
          .map((ev) => {
        ...
        newUsersEvents[currentlyCalendarProcessingUser.username] =
          calendarWeekEvents;
  ...
  const mergeTimes = (usersEvents, weekStart, weekEnd) => {
    ...
    for (const user of users) {
      ...
      usersEvents[user.username] = usersEvents[user.username]?.sort(
        (ev1, ev2) => ev1.start.getTime() - ev2.start.getTime()
      );
    ...
    for (const user of users) {
      ...
      if (
        userConsolidatedBusyEvents.length === 0 ||
        userConsolidatedFreeEvents.length === 0
      ) {
        userConsolidatedEvents = userConsolidatedBusyEvents;
      } else {
        // remove free blocks from busy blocks
        let freeEventIndex = 0;
        let busyEventIndex = 0;
        while (
          freeEventIndex < userConsolidatedFreeEvents.length &&
          busyEventIndex < userConsolidatedBusyEvents.length
        ) {
        ...
        while (busyEventIndex < userConsolidatedBusyEvents.length) {
          userConsolidatedEvents.push(
            userConsolidatedBusyEvents[busyEventIndex++]
          );
        }
      }
    ...
    while (currEvent.start.getTime() < weekEnd.getTime()) {
      ...
      for (const user of users) {
        ...
        if (
          !earliestEvent ||
          earliestEvent.start.getTime() > event.start.getTime()
        ) {
          earliestEvent = event;
      ...
      if (currEvent.usersBusy.length === 0) {
        ...
        currEvent = {
          usersBusy: [earliestEvent.username],
          start: earliestEvent.start,
          end: earliestEvent.end,
        };
        ...
        if (earliestEvent.toRemove) {
          ...
          const toRemoveIndex = currEvent.usersBusy.indexOf(
            earliestEvent.username
          );
          currEvent.usersBusy = currEvent.usersBusy
            .slice(0, toRemoveIndex)
            .concat(
              currEvent.usersBusy.slice(
                toRemoveIndex + 1,
                currEvent.usersBusy.length
              )
            );
  ...
  const createMergedSchedule = (mergedEvents) => {
    setMergedCustomEvents(
      mergedEvents.map((event) => {
        const percentageBusy = event.usersBusy.length / users.length;
        return {
          id: Math.random(),
          title: `${users.length - event.usersBusy.length}/${
            users.length
          } available`,
          startDate: event.start,
          endDate: event.end,
          customColor: `rgb(${255 * percentageBusy}, ${
            128 * (1 - percentageBusy)
          }, 0)`,
          tooltip: `${
            event.usersBusy.length < users.length
              ? `Available (${users.length - event.usersBusy.length}): ${users
                  .filter((user) => event.usersBusy.indexOf(user.username) < 0)
                  .map((user) => user.username)
                  .join(", ")}<br>`
              : ""
          }${
            event.usersBusy.length > 0
              ? `Unavailable (${
                  event.usersBusy.length
                }): ${event.usersBusy.join(", ")}`
              : ""
          }`,
        };
      })
  ...
  const processNextUsersEvents = (usersEvents, weekStart, weekEnd) => {
    ...
    let nextUserToProcessIndex =
      users.findIndex(
        (user) => user?.username === currentlyCalendarProcessingUser?.username
      ) + 1;
